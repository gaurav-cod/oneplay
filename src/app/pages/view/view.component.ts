  ...
  playGame(container): void {
    ...
    this.restService.getTokensUsage().subscribe((data) => {
      ...
      if (data.total_tokens === 0) {
        swal_html = `Looks like your gaming subscription has expired, and it's time to renew to keep the adventure going! <p class="mt-4 "><a href="${this.domain}/subscription.html" class="btn playBtn border-0 text-white GradientBtnPadding">Buy Now</a></p>`;
      }
      else if(data.total_tokens > 0 && data.remaining_tokens < 1) {
        swal_html = `Your game time has run out. Time to recharge and get back into the action. <p class="mt-4 "><a href="${this.domain}/subscription.html" class="btn playBtn border-0 text-white GradientBtnPadding">Buy Now</a></p>`;
      }
      else {
